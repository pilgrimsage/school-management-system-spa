...
<script>
    ...
    const pluginConfigs = {
        dataTable: {
            ...
            routes: ['dashboard', 'student/assignments', 'student/grades', 'student/attendance',
                'student/schedule', '*'
            ],
            init: initDataTable,
    ...
    function initDataTable(route) {
        ...
        $('table.display, .datatable, #assignmentTable, #reportCardTable, #gradesTable, #attendanceTable').each(function () {
            try {
    ...
    function destroyCharts() {
        if (window.ApexCharts) {
            const chartElements = ['#complete-course', '#earned-certificate', '#course-progress',
                '#community-support', '#performanceChart', '#attendanceChart', '#doubleLineChart', '#radialMultipleBar'
            ];
    ...
    function bindGlobalEventListeners() {
        ...
        // ✅ CORRECT: Unbind delegated events from document
        $(document).off("click", "#logoutBtn");
        $(document).off("click", "a.nav_js");
        $(document).off("click", ".nav_js");

        console.log('Global event listeners unbound');
    ...
    function downloadJSON(data) {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'student_export_' + new Date().toISOString().split('T')[0] + '.json';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
    ...
    document.addEventListener('visibilitychange', function () {
        if (!document.hidden) {
            // Optional refresh logic when page becomes visible
        }
    });
    ...
    console.log('✅ Student SPA Initialized');
</script>
